...
import { Step } from '../../src/steps/email-links-validation';

chai.use(sinonChai);

describe('EmailLinksValidationStep', () => {
  ...
  describe('Metadata', () => {
    ...
    it('should return expected step metadata', () => {
      const stepDef: StepDefinition = stepUnderTest.getDefinition();
      expect(stepDef.getStepId()).to.equal('EmailLinksValidationStep');
      expect(stepDef.getName()).to.equal('Check that no link in an email is broken');
      expect(stepDef.getExpression()).to.equal('the (?<position>\\d+)(?:(st|nd|rd|th))? mailgun email for (?<email>.+) should not contain broken links');
